...
resource "azurerm_lb_rule" "named_test_resource" {
  ...
  frontend_port                  = 3389
  backend_port                   = 3389
  frontend_ip_configuration_name = var.resource_name
}

output "resource_aka" {
  depends_on = [azurerm_lb_rule.named_test_resource]
  value      = "azure://${azurerm_lb_rule.named_test_resource.id}"
}

output "resource_aka_lower" {
  value = "azure://${lower(azurerm_lb_rule.named_test_resource.id)}"
}
...
output "resource_id" {
  value = azurerm_lb_rule.named_test_resource.id
}
